...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    conditionalPanel(
      ...
      tags$div(
        style = "margin-top: 15px; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #365B6D; border-radius: 4px;",
    ...
    conditionalPanel(
      ...
      tags$div(
        style = "margin-top: 15px; padding: 10px; background-color: #007bff; border-left: 4px solid #8FAB9A; border-radius: 4px;",
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # Nueva paleta de colores basada en tu esquema
    paleta <- colorBin(
  ...
  output$serieTiempo <- renderPlot({
    ...
    if (nrow(df) == 0) {
      ...
      ggplot(df, aes(x = mes_fecha, y = total_delitos)) +
        geom_area(fill = "#8FAB9A", alpha = 0.4) +
        geom_line(color = "#365B6D", size = 1.1, alpha = 0.8) +
        geom_point(color = "#6C9286", size = 1.3, alpha = 0.9) +
        scale_y_continuous(
        ...
        theme_minimal() +
        theme(
          axis.text.x = element_text(angle = 45, hjust = 1, family = "Segoe UI", size = 9, color = "#365B6D"),
          axis.text.y = element_text(family = "Segoe UI", size = 9, color = "#365B6D"),
          axis.title.x = element_text(family = "Segoe UI", size = 12, color = "#365B6D", face = "bold"),
          axis.title.y = element_text(family = "Segoe UI", size = 12, color = "#365B6D", face = "bold"),
          plot.title = element_text(family = "Segoe UI", size = 18, face = "bold", hjust = 0.5, color = "#365B6D"),
          panel.grid.major = element_line(color = "#ECF0F1", size = 0.5),
          panel.grid.minor = element_line(color = "#F8F9FA", size = 0.3),
          plot.background = element_rect(fill = "#F2F1EC", color = NA),
          panel.background = element_rect(fill = "#F8F9FA", color = NA),
          axis.line = element_line(color = "#7E8B97", size = 0.5)
        )
    }
  })
  ...
  output$topBarrios <- renderPlot({
    ...
    if (nrow(top_barrios) == 0) {
      ...
      ggplot(top_barrios, aes(x = barrio, y = total_delitos)) +
        geom_col(fill = "#6C9286", alpha = 0.8, width = 0.7) +
        geom_text(aes(label = format(total_delitos, big.mark = ".", decimal.mark = ",")), 
                  hjust = -0.1, size = 3, color = "#365B6D", fontface = "bold") +
        coord_flip() +
        scale_y_continuous(
        ...
        theme_minimal() +
        theme(
          axis.text.x = element_text(family = "SegoeUI", size = 7, color = "#365B6D", angle = 0),
          axis.text.y = element_text(family = "SegoeUI", size = 11, color = "#365B6D"),
          axis.title.x = element_text(family = "SegoeUI", size = 10, color = "#365B6D", face = "bold"),
          plot.title = element_text(family = "SegoeUI", size = 12, face = "bold", hjust = 0.5, color = "#365B6D"),
  ...
  output$topTipos <- renderPlot({
    ...
    if (nrow(top_tipos) == 0) {
      ...
      ggplot(top_tipos, aes(x = tipo, y = total_delitos)) +
        geom_col(fill = "#7E8B97", alpha = 0.8, width = 0.7) +
        geom_text(aes(label = format(total_delitos, big.mark = ".", decimal.mark = ",")), 
                  hjust = -0.1, size = 3, color = "#365B6D", fontface = "bold") +
        coord_flip() +
        scale_y_continuous(
        ...
        theme_minimal() +
        theme(
          axis.text.x = element_text(family = "SegoeUI", size = 8, color = "#365B6D"),
          axis.text.y = element_text(family = "SegoeUI", size = 11, color = "#365B6D"),
          axis.title.x = element_text(family = "SegoeUI", size = 10, color = "#365B6D", face = "bold"),
          plot.title = element_text(family = "SegoeUI", size = 12, face = "bold", hjust = 0.5, color = "#365B6D"),
          panel.grid.major.y = element_blank(),
